...
import { useTradeEvents } from "../../hooks/useEvents";
...
const TradeEventItem = ({ event }: ItemProps) => {
  ...
  return (
    <div className="flex justify-between my-2 py-3 text-left w-big">
      ...
      <div className="w-full">
        <SideTypeStacked side={event.optionSide} type={pool.optionType} />
      </div>
      <div className="w-full">
        <P3 className="font-semibold">
          {pool.underlying.symbol}
          {event.strikePrice.val}
        </P3>
      </div>
      <div className="w-full">
        <MaturityStacked timestamp={event.maturity} />
      </div>
      <div className="w-full">
...
const TradeEventsWithUser = ({ user }: Props) => {
  const [page, setPage] = useState<number>(1);
  const pageSize = 10;
  const { isLoading, isError, data } = useTradeEvents(
    user,
    pageSize,
    (page - 1) * pageSize
  );
...
export const TradeEvents = () => {
  ...
  return <TradeEventsWithUser user={address} />;
};
